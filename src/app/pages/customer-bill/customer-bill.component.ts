...
export class CustomerBillComponent implements OnInit {
  ...
  updateProduct(i: number) {
    ...
    // 1. Calculate after discount
    p.afterDisc = +(p.basePrice * (1 - (p.discount || 0) / 100)).toFixed(2);
    // 2. GST Amount
    p.gstAmt = +((p.afterDisc * (p.gst || 0)) / 100).toFixed(2);
    // 3. Final Rate
    p.finalRate = +(p.afterDisc + p.gstAmt).toFixed(2);
  }
  addProduct() {
  ...
  calculateSummary(products: any[]) {
    ...
    products.forEach((p, i) => {
      const base = (p.mrp || 0) * (p.qty || 1);
      const priceAfterDiscount = base * (1 - (p.discount || 0) / 100);
      const gstAmount = (priceAfterDiscount * (p.gst || 0)) / 100;
      const finalRate = priceAfterDiscount + gstAmount;
      const total = finalRate;
  ...
  buildPdfDefinition(summary: any) {
    return {
      content: [
        ...
        // Billed To (Buyer) Block
        // {
        //   text: [
        //     { text: "Billed To (Buyer):\n", bold: false, fontSize: 10 },
        //     { text: "Customer Name: ", bold: true, fontSize: 10 },
        //     { text: this.form.customerName + "\n", fontSize: 10 },
        //     { text: "Address: ", bold: true, fontSize: 10 },
        //     { text: this.form.address + "\n", fontSize: 10 },
        //     { text: "Mobile No.: ", bold: true, fontSize: 10 },
        //     { text: this.form.mobile + "\n", fontSize: 10 },
        //     { text: "Drug License No.: ", bold: true, fontSize: 10 },
        //     { text: (this.form.drugLicense || "[Not Provided]") + "\n", fontSize: 10 },
        //     { text: "PAN No.: ", bold: true, fontSize: 10 },
        //     { text: (this.form.pan || "[Not Provided]"), fontSize: 10 }
        //   ],
        //   margin: [0, 0, 0, 10]
        // },

        // Detailed Price Calculation heading
        {
          ...
          table: {
            ...
            body: [
              [
                ...
                { text: 'MRP', bold: true, fontSize: 11 },
                { text: 'Base Price', bold: true, fontSize: 11 },
                { text: 'Discount %', bold: true, fontSize: 11 },
                { text: 'After Disc.', bold: true, fontSize: 11 },
                { text: '+18% GST', bold: true, fontSize: 11 },
                { text: 'Final Rate', bold: true, fontSize: 11 },
              ],
              ...this.form.products.map((p: any) => [
                p.name,
                p.mrp.toFixed(2),
                p.basePrice.toFixed(2),
                `${p.discount}%`,
                p.afterDisc.toFixed(2),
                p.gstAmt.toFixed(2),
                p.finalRate.toFixed(2),
              ]),
            ],
          },
          fontSize: 10,
        ...
        // Invoice item table (No, Product Description, Qty, Rate, Total)
        {
          table: {
            ...
            body: [
              [
                { text: 'No', bold: true, fontSize: 11 },
                { text: 'Product Description', bold: true, fontSize: 11 },
                { text: 'Qty', bold: true, fontSize: 11 },
                { text: 'Rate', bold: true, fontSize: 11 },
                { text: 'Total', bold: true, fontSize: 11 },
              ],
              ...this.form.products.map((p: any, i: any) => [
                i + 1,
                `HerbMudita ${p.name} - Ayurvedic`,
                p.qty,
                p.finalRate.toFixed(2),
                (p.finalRate * p.qty).toFixed(2),
              ]),
            ],
          },
          fontSize: 10,
        ...
        },
        {
          ...
          stack: [
            {
              text: `Taxable Value: ${summary.taxableValue.toFixed(2)}`,
              fontSize: 10,
              margin: [0, 0, 0, 2],
            },
            {
              text: `CGST @${summary.cgstPercent}%: ${summary.cgst.toFixed(2)}`,
              fontSize: 10,
              margin: [0, 0, 0, 2],
            },
            {
              text: `SGST @${summary.sgstPercent}%: ${summary.sgst.toFixed(2)}`,
              fontSize: 10,
              margin: [0, 0, 0, 8],
            },
            {
            ...
            },
            {
              text: [
                'Note: Prices include ',
                { text: '40% discount', bold: true },
                " under scheme 'MEDICOPARTNER' on base value.\nApplicable GST @18% is charged on discounted base price.",
              ],
              fontSize: 10,
              margin: [0, 15, 0, 10],
            },
